...
<body style="margin-top: 10px">
	<div class="container">
		...
		<%
					...
					if (rs.next()) {
				%>
		<form action="../articleUpdate?id=<%=rs.getInt("id") %>" method="post">
			<div class="input-group">
			
				<span class="input-group-btn">
					<button class="btn btn-default" type="button">Title</button>
				</span> <input type="text" class="form-control" name="title" value="<%=rs.getString("title") %>"> <span
					class="input-group-btn">
					<button class="btn btn-default" type="button">Author</button>
				</span> <input type="text" class="form-control" name="author" disabled value="<%=rs.getString("author") %>">
			<span class="input-group-btn">
					<button class="btn btn-default" type="button">Modular</button>
				</span> 
				<select class="form-control" name="modular">
						<%
					ResultSet rs2=con.getSelect("select * from modular");
					while(rs2.next()){
						%>
						<option value="<%=rs2.getString("modularName")%>"><%=rs2.getString("modularName")%></option>
						<%
					}
					rs2.close();		
				%>
